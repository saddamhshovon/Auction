    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="grid grid-cols-2 gap-4">
        <div class="card text-center shadow-xl rounded-none h-72">
          <figure class="pt-10 object-contain h-48 w-full">
            <img
              :src="auction.front_image"
              class="rounded-none object-contain h-32 w-full"
            />
          </figure>
          <div class="card-body">
      ...
      <div class="card lg:card-side bordered rounded-none">
        <div class="card-body">
          ...
          <h2 v-if="can_bid" class="text-lg font-bold">
            Highest Bidder: {{ auction.user_name }}
          </h2>
          <h2 v-if="can_bid" class="text-lg font-bold">
            Highest Bid: {{ auction.winner_bid }}
          </h2>
          ...
          <div class="card-actions mt-10" v-if="can_bid">
            ...
            <button
              ...
              :class="
                btn_loading
                  ? 'ml-10 btn btn-primary loading rounded-none'
                  : 'ml-10 btn btn-primary rounded-none'
              "
            >
              Bid
            </button>
      ...
      AuctionService.getauctiondetails(this.auctionid)
        .then((response) => {
          ...
          if (this.auction.front_image !== null) {
            this.auction.front_image =
              env.baseURL + "/" + this.auction.front_image;
          } else {
            this.auction.front_image = "/images/pre-upload.png";
          }
          if (this.auction.back_image !== null) {
            this.auction.back_image =
              env.baseURL + "/" + this.auction.back_image;
          } else {
            this.auction.back_image = "/images/pre-upload.png";
          }
          if (this.auction.left_image !== null) {
            this.auction.left_image =
              env.baseURL + "/" + this.auction.left_image;
          } else {
            this.auction.left_image = "/images/pre-upload.png";
          }
          if (this.auction.right_image !== null) {
            this.auction.right_image =
              env.baseURL + "/" + this.auction.right_image;
          } else {
